...
this post covers the theory and findings of my bachelor thesis. during the last few months, i did a deep dive into (encoder-)transformers, alignment in multimodal transformers, and representational similarity. here, i show my findings and other interesting stuff i encountered during my research.

## high-level overview

during the work on my thesis, many people asked me about its topic. when trying to explain it even to other cs students i struggled, as it is a pretty specialized topic. not to mention how hard it was to explain it to non-cs people. therefore, i want to start with a high-level overview of the topic.

in a nutshell, my research covers the question: *how to bring together vision and language*?
our brain brings together different senses to form the experience of the world. for example, when we *eat* an apple, we *see* the red color, *smell* the distinct apple scent, *taste* the sweet flavor, et cetera.
...
multiple modalities of the same concept (e.g., image and text of an apple) can provide complementary information. this means that each modality conveys idiosyncratic information that the other modality does not. by fusing them together, the resulting representation captures both modalities. in some cases, the additional information is only available by combining both modalities. an example of this is memes with ambiguous jokes. here, one modality alone does not suffice to understand the joke. the combination of image and text is necessary to understand the joke.
...
the goal is to extract the low dimensional manifold using encoders.
...
more recent architectures like bert use the attention mechanism in transformers to encode text. using two pretraining techniques, the transformer implicitly learns to encode text to semantically meaningful vectors.
basically, the transformer learns to transform representations to a more suitable form for downstream tasks.
in a transformer like bert, multiple layers are used. each layer extracts features, which are transformed by the next layer.
for example in bert base, 12 layers are used, transforming the input data to a new representation. the last representation is used for tasks like classification and embeddings for vector databases.

### early vs. late fusion

the vilbert architecture (by lu et al., 2019) is inspired by the brain. our brain processes sensory data before integrating them together.
separate pretrained encoder models process each modality before integrating them into one representation.
...
the above figure depicts the dual-stream architecture. up to layer three, the modalities extract features independently. layer four fuses both modalities together. while independent feature extraction is performed using *self-attention*, fusion is achieved by *cross-attention*.
...
$$
\operatorname{softmax}\left(\frac {Q_V^TK_W} {\sqrt d} \right)V_W
$$
...
in multimodal theory, modalities are on a spectrum with the extremes of *raw* and *abstract*. input data is raw, while encoded/processed data is abstract. early fusion fuses raw data, while late fusion fuses abstract data. middle fusion is in between.
